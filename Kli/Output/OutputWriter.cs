...
namespace Kli.Output
{
    ...
    public class OutputWriter : IOutputWriter
    {
        ...
        private IEnumerable<Tuple<string, char>> ExtractParts(string text)
        {
            ...
            var indexes = Regex
                .Matches(text, $"[{_outputMarkers.MarkersEscapedForRegexJoined}]", RegexOptions.Singleline)
        ...
        private string RemoveAllMarkers(string text)
        {
            ...
            if (!Regex.IsMatch(text, $@"[{_outputMarkers.MarkersEscapedForRegexJoined}]", RegexOptions.Singleline))
                return text;
            ...
            for (var i = 0; i < _outputMarkers.Markers.Length; i++)
            {
                ...
                text = Regex.Replace(text, $@"(?<!{markerEscaped})({markerEscaped})(?!{markerEscaped})", string.Empty,
                ...
                while (position >= 0)
                {
                    if (text[position + 1] == marker) position++;
